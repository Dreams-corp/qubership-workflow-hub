...
async function run() {
  ...
  const configurationPath =
    core.getInput("configuration-path") || defaultConfigurationPath;
  ...
  if (count > assigneesLength) {
    core.warning(
      `Assignees count ${count} exceeds number of available assignees ${assigneesLength}. Using available count (${assigneesLength}).`,
    );
  ...
  try {
    ...
    if (currentAssignees != "") {
      core.info(
        `💡✔️ PR has current assignees: ${currentAssignees}, skipping...`,
      );
      return;
    }
